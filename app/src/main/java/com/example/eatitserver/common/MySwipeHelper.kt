...
abstract class MySwipeHelper(
    ...
    abstract fun instaniateMyButton(
    ...
    private val onTouchListener = View.OnTouchListener() { view, motionEvent ->
        ...
        val swipeViewHolder = recyclerView.findViewHolderForAdapterPosition(swiptePosition)
        ...
        if (motionEvent.action == MotionEvent.ACTION_DOWN || motionEvent.action == MotionEvent.ACTION_UP || motionEvent.action == MotionEvent.ACTION_MOVE) {
            if (rect.top < point.y && rect.bottom > point.y)
                ...
                removerQueue!!.add(swiptePosition)
                swiptePosition = -1
                recoverSwipedItem()
            }
        }
    ...
    override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
        ...
        if (swiptePosition != pos)
            removerQueue.add(swiptePosition)
        swiptePosition = pos
        if (buttonBuffer.containsKey(swiptePosition))
            buttonList = buttonBuffer[swiptePosition]
        else
            buttonList!!.clear()
        buttonBuffer.clear()
        ...
        if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
            if (dX < 0) {
                ...
                if (!buttonBuffer.containsKey(pos)) {
                    instaniateMyButton(viewHolder, buffer)
