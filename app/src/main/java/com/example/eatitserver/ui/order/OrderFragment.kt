...
import com.example.eatitserver.adapter.MyOrderAdapter
import com.example.eatitserver.callback.IMyButtonCallback
import com.example.eatitserver.common.BottomSheetOrderFragment
...
import com.example.eatitserver.eventbus.LoadOrderEvent
...
class OrderFragment : Fragment() {
    ...
    lateinit var fcmServices: IFCMServices
    ...
    private fun initViews(root: View?) {
        fcmServices = RetrofitFCMClient.getInstance().create(IFCMServices::class.java)
        ...
        var swipe = object : MySwipeHelper(context!!, recycler_order!!, width / 6) {
    ...
    private fun showEditDialog(orderModel: OrderModel, pos: Int) {
        ...
        if (orderModel.orderStatus == -1) {
            layout_dialog =
                LayoutInflater.from(context!!).inflate(R.layout.layout_dialog_cancelled, null)
            builder = AlertDialog.Builder(context!!).setView(layout_dialog)
            rdi_restore_placed =
            ...
            layout_dialog =
                LayoutInflater.from(context!!).inflate(R.layout.layout_dialog_shipping, null)
            ...
            layout_dialog =
                LayoutInflater.from(context!!).inflate(R.layout.layout_dialog_shipped, null)
            builder = AlertDialog.Builder(context!!).setView(layout_dialog)
        ...
        txt_status.setText(
            StringBuilder("Order Status (").append(
                Common.convertStatusToString(
        ...
        if (!TextUtils.isEmpty(orderModel.key)) {
            FirebaseDatabase.getInstance()
                ...
                .addOnSuccessListener {
                    ...
                    adapter!!.notifyItemRemoved(pos)
                    updateTextCounter()
                    Toast.makeText(context!!, "Update Order success!!", Toast.LENGTH_SHORT).show()
                            ...
                            override fun onDataChange(p0: DataSnapshot) {
                                if (p0.exists()) {
                                    ...
                                    compositeDisposable.add(
                                        fcmServices.sendNotification(sendData)
    ...
    private fun updateTextCounter() {
        txt_order_filter.setText(StringBuilder("Order (").append(adapter!!.itemCount).append(")"))
    }
